...
useEffect(() => {
  ...
  const fetchStats = async () => {
    setLoading(true);
    try {
      const res = await axios.get('https://kaba-abn-api.kabatitude.com/dashboard/history', { params: { debut: periode.debut, fin: periode.fin } });
      setStats(res.data);
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard
            ...
            value={stats?.nbAbonnements ?? '0'}
            icon={Users}
            color="blue"
          />
          <StatCard
            title="Chiffre d'affaires"
            value={stats?.chiffreAffaires ? `XOF${stats.chiffreAffaires}` : '0'}
            icon={DollarSign}
            color="green"
          />
          <StatCard
            title="Frais de livraison"
            value={stats?.fraisLivraison ? `XOF${stats.fraisLivraison}` : '0'}
            icon={CreditCard}
            color="orange"
          />
          <StatCard
            title="Nombre de commandes"
            value={stats?.nbCommandes ?? '0'}
            icon={BarChart3}
            color="purple"
          />
        </div>
